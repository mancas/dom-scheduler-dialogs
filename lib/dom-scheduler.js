...
(function(exports) {
  ...
  var directProtectionWindow = 300;
  ...
  scheduler.prototype = {
    ...
    // flush.
    // They will also be delayed by `direct` blocks except when the `feedback`
    // flag is true, in that case they have the same priority as `direct`
    // blocks.
    //
    ...
    _transitionFactory: function(transition) {
      return this.transition(transition.block, transition.elm,
        transition.evt, transition.timeout, transition.feedback);
    },

    multipleTransitions: function (transitions) {
      var result = Promise.resolve();
      transitions.forEach(transition => {
        result = result.then(this._transitionFactory.bind(this, transition));
      });
      return result;
    },
